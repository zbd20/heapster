...
func newClient(c *Config) (*sls.Client, error) {
	if c.regionId == "" || c.accessKeyId == "" || c.accessKeySecret == "" {
		glog.Info("accessKeyId,accessKeySecret or regionId is empty and use metadata instead.")

		m := metadata.NewMetaData(nil)
		regionId, err := m.Region()
		if err != nil {
			glog.Errorf("failed to get Region,because of %s", err.Error())
			return nil, err
		}

		roleName, err := m.RoleName()
		if err != nil {
			glog.Errorf("failed to get RoleName,because of %s", err.Error())
			return nil, err
		}

		auth, err := m.RamRoleToken(roleName)
		if err != nil {
			glog.Errorf("failed to get RamRoleToken,because of %s", err.Error())
			return nil, err
		}
